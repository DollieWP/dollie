...
namespace Dollie\Core\Admin\NavMenu;
...
class Component {
	...
	public function __construct() {
		if ( is_admin() ) {
			require_once DOLLIE_CORE_PATH . 'Admin/NavMenu/Ajax_Walker_Nav_Menu_Checklist.php';
		}
	...
	public function enqueue_scripts() {
		...
		wp_localize_script( 'dollie-notifications', 'wpdNotifications', [
		...
		switch ( $matches[1] ) {
			...
			case 'avatar' :
				if ( ! is_user_logged_in() ) {
					...
					$avatar           = get_avatar_url( get_current_user_id(), array(
						'width'  => 30,
						'height' => 30,
					) );
	...
	private function get_notifications_content( $args = [] ) {
		...
		return [ 'count' => $count, 'content' => $item_output ];
	}
	...
	public function nav_notifications( $item_output, $item, $depth, $args ) {
		if ( in_array( 'dollie-notifications-nav', $item->classes, true ) ) {
			...
			add_action( 'wp_footer', static function () use ( $footer_output ) {
				echo $footer_output;
			}, 20 );
