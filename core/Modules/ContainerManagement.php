...
class ContainerManagement extends Singleton {
	...
	public function register_container() {
		$labels = [
			...
			'menu_name'             => __( 'Containers', DOLLIE_SLUG ),
			'name_admin_bar'        => __( 'Post Type', DOLLIE_SLUG ),
			'archives'              => __( 'Container Archives', DOLLIE_SLUG ),
			'attributes'            => __( 'Container Attributes', DOLLIE_SLUG ),
			'parent_item_colon'     => __( 'Parent Container:', DOLLIE_SLUG ),
			'all_items'             => __( 'View Containers', DOLLIE_SLUG ),
			'add_new_item'          => __( 'Add New Container', DOLLIE_SLUG ),
			'add_new'               => __( 'Add New', DOLLIE_SLUG ),
			'new_item'              => __( 'New Container', DOLLIE_SLUG ),
			'edit_item'             => __( 'Edit Container', DOLLIE_SLUG ),
			'update_item'           => __( 'Update Container', DOLLIE_SLUG ),
			'view_item'             => __( 'View Container', DOLLIE_SLUG ),
			'view_items'            => __( 'View Containers', DOLLIE_SLUG ),
			'search_items'          => __( 'Search Containers', DOLLIE_SLUG ),
			'not_found'             => __( 'Not found', DOLLIE_SLUG ),
			'not_found_in_trash'    => __( 'Not found in Trash', DOLLIE_SLUG ),
			'featured_image'        => __( 'Featured Image', DOLLIE_SLUG ),
			'set_featured_image'    => __( 'Set featured image', DOLLIE_SLUG ),
			'remove_featured_image' => __( 'Remove featured image', DOLLIE_SLUG ),
			'use_featured_image'    => __( 'Use as featured image', DOLLIE_SLUG ),
			'insert_into_item'      => __( 'Insert into item', DOLLIE_SLUG ),
			'uploaded_to_this_item' => __( 'Uploaded to this item', DOLLIE_SLUG ),
			'items_list'            => __( 'Containers list', DOLLIE_SLUG ),
			'items_list_navigation' => __( 'Containers list navigation', DOLLIE_SLUG ),
			'filter_items_list'     => __( 'Filter items list', DOLLIE_SLUG ),
		];

		$args = [
			'label'              => __( 'Container', DOLLIE_SLUG ),
			'description'        => __( 'Platform Containers', DOLLIE_SLUG ),
			'labels'             => $labels,
	...
	public function get_customer_container_details() {
		...
		if ( empty( $request ) ) {
			...
			update_post_meta( $post_id, 'wpd_container_user', $update_response['containerSshUsername'] );
			update_post_meta( $post_id, 'wpd_container_port', $update_response['containerSshPort'] );
			update_post_meta( $post_id, 'wpd_container_password', $update_response['containerSshPassword'] );
			update_post_meta( $post_id, 'wpd_container_ip', preg_replace( '/\s+/', '', $update_response['containerHostIpAddress'] ) );
			update_post_meta( $post_id, 'wpd_container_deploy_time', $update_response['deployedAt'] );
			update_post_meta( $post_id, 'wpd_container_uri', $update_response['uri'] );
		}
	...
	public function container_api_request( $url, $transient_id, $user_auth, $user_pass ) {
		...
		$post_slug = get_queried_object()->post_name;
		$request   = get_transient( 'dollie_container_api_request_' . $post_slug . '_' . $transient_id );
		...
		if ( empty( $request ) ) {
			...
			set_transient( 'dollie_container_api_request_' . $post_slug . '_' . $transient_id, $request, MINUTE_IN_SECONDS * 30 );
		}
	...
	public function fetch_container_details() {
		if ( isset( $_GET['get-details'] ) ) {
			$post_slug = get_queried_object()->post_name;
			delete_transient( 'dollie_s5_container_details_' . $post_slug );
