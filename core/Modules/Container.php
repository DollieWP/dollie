...
class Container extends Singleton {
	...
	public function __construct() {
		...
		add_action( 'wp_footer', [ $this, 'site_demo_notice' ] );
	...
	public function site_demo_notice() {
		$deploying = 'pending' === self::instance()->get_status();
		$post_id   = get_the_ID();

		if ( ! is_singular( 'container' ) || ! dollie()->is_site_demo( $post_id ) ) {
			return;
		}
		?>
		...
		<div class="dol-fixed dol-w-full dol-bg-accent dol-p-3 dol-text-white dol-bottom-0 dol-left-0 dol-z-50 dol-text-center">
			<a class="dol-text-white hover:dol-text-white" href="<?php echo get_permalink() . '/blueprints'; ?>">
				<?php echo dollie()->get_icon_site(); ?><strong> <?php printf( __( 'Demo %1$s', 'dollie' ), dollie()->get_site_type_string() ); ?></strong> -
				<?php printf( __( 'This %1$s is not part of your platform but serves as an example so you can view your awesome dashboard. Pretty neat huh?', 'dollie' ), dollie()->get_site_type_string() ); ?>
			</a>
		</div>
		<?php
	}
