...
class Blueprints extends Singleton
{
	...
	public function blueprint_notice()
	{
		global $wp_query;
		$id = $wp_query->post->ID;
		if (dollie()->is_blueprint_staging($id)) { ?>
			<div class="dol-bg-gray-700 dol-p-3 dol-text-white dol-inset-x-0 dol-bottom-0 dol-z-50 dol-text-center dol-custom-notification">
				<i class="fas fa-copy"></i> <strong>
					<a class="dol-text-white" href=" <?php echo get_permalink(); ?>/blueprints"><?php esc_html_e('Staging', 'dollie'); ?></strong> - <?php esc_html_e('This Blueprint is still in staging mode. Click here to make it available for your customers.', 'dollie'); ?></a>
			</div>
		<?php } else {
			$blueprint_time = get_post_meta($id, 'wpd_blueprint_time', true);
		?>
			<div class="dol-bg-secondary dol-p-3 dol-text-white dol-inset-x-0 dol-bottom-0 dol-z-50 dol-text-center dol-custom-notification">
				<a class="dol-text-white" href="<?php echo get_permalink(); ?>/blueprints">
					<i class="fas fa-copy"></i> <strong><?php esc_html_e( 'Live', 'dollie' ); ?></strong> - <?php printf(__('This Blueprint was last updated at %s', 'dollie'), $blueprint_time); ?>
				</a>
			</div>
		<?php } ?>
<?php
