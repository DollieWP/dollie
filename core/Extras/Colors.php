...
function dollie_add_to_access_group($group_id, $user_ids) {
    ...
    // If there are no current users, initialize to an empty array
    if (!is_array($current_users)) {
        $current_users = array();
    }

    // Merge current users and new users
    $updated_users = array_unique(array_merge($current_users, $user_ids));

    // Update the field with the new user list
    update_field('wpd_group_users', $updated_users, $group_id);
}


function dolie_remove_from_access_group($group_id, $user_ids) {
    // Convert single user ID to array
    if (!is_array($user_ids)) {
        $user_ids = array($user_ids);
    }
    ...
    // If there are no current users, just return
    if (!is_array($current_users)) {
        return;
    }
    ...
    // Update the field with the new user list
    update_field('wpd_group_users', array_values($updated_users), $group_id);
}

function dolie_user_has_access_to_group($group_id, $user_id) {
    // Assuming the field name is 'wpd_group_users'
    $current_users = get_field('wpd_group_users', $group_id);
...
function dolie_get_users_in_group($group_id) {
    $current_users = get_field('wpd_group_users', $group_id);
    return is_array($current_users) ? $current_users : [];
}

function dolie_is_group_empty($group_id) {
    $current_users = get_field('wpd_group_users', $group_id);
    return empty($current_users);
}

function dolie_count_users_in_group($group_id) {
    $current_users = get_field('wpd_group_users', $group_id);
...
function dol_theme_body_close() {
	...
	dolie_remove_from_access_group(364, 2);
	dollie_add_to_access_group(364, 2);
	// Assuming 364 is the group ID and 2 is the user ID

	// if (dolie_user_has_access_to_group(364, get_current_user_id())) {
	// 	echo "User 2 has access to group 364";
	// } else {
	// 	echo "User 2 does not have access to group 364";
	// }
