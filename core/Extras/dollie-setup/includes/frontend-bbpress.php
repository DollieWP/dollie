...
class CBox_BBP_Autoload {
	 ...
	 *
	 * @see https://github.com/cuny-academic-commons/dollie-setup/issues/76
	 */
	 ...
	 * emulate teeny mode.
	 *
	 * @see https://github.com/cuny-academic-commons/dollie-setup/issues/91
	 */
	...
	public function show_notice_for_moderated_posts() {
		...
		$notice = function( $retval ) {
			...
			$msg = esc_html__( 'Your forum post is pending moderation', 'dollie-setup' );
	...
	public function highlight_unread_posts() {
		...
		add_filter( 'bbp_reply_admin_links', function( $retval ) {
			...
			if ( bbp_get_reply_id() >= (int) $_GET['bbp-unread'] ) {
				$retval['unread'] = sprintf( '<span class="bbp-unread-new">%s</span>', esc_html__( 'New', 'dollie-setup' ) );
			}
		...
		$notifications_header = function() {
			...
			switch ( $_GET['type'] ) {
				case 'bbp_new_reply' :
					$title = esc_html__( 'Filtered by forum posts', 'dollie-setup' );
			...
			$reset = esc_html__( 'Reset', 'dollie-setup' );
