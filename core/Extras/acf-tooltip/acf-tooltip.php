...
if ( ! class_exists( 'dhz_acf_plugin_tooltip' ) ) :
	...
	class dhz_acf_plugin_tooltip {
		...
		function __construct() {
			...
			add_action( 'admin_init', [ $this, 'acf_or_die' ], 11 );

			// include field
			add_action( 'acf/input/admin_enqueue_scripts', [ $this, 'acf_tooltip_enqueue_scripts' ], 11 );
			...
			if ( DHZ_SHOW_DONATION_LINK == true ) {
				...
				add_filter( '_dhz_plugins_list', [ $this, '_dhz_meta_box_data' ] );

				// metabox callback for plugins list and donation link
				add_action( 'add_meta_boxes_acf-field-group', [ $this, '_dhz_plugins_list_meta_box' ] );
		...
		function kill_plugin() {
			...
			add_action( 'admin_notices', [ $this, 'acf_dependent_plugin_notice' ] );
		}
		...
		*/
		function acf_tooltip_enqueue_scripts() {
			...
			wp_register_script( 'acf-tooltip-qTip2-script', "{$url}assets/vendor/qtip/jquery.qtip.min.js", [ 'jquery' ], '3.0.3', true );

			// register ACF Tooltip script
			wp_register_script( 'acf-tooltip-script', "{$url}assets/js/acf-tooltip.js", [ 'jquery' ], $version, true );
		...
		*  Add plugin to $plugins array for the metabox
		*/
		function _dhz_meta_box_data( $plugins = [] ) {
		...
		*/
		function _dhz_plugins_list_meta_box() {
			...
			$plugins = apply_filters( '_dhz_plugins_list', [] );
			...
			$callback = [ $this, 'show_dhz_plugins_list_meta_box' ];
			$screens  = [];
			...
			<ul style="margin-top: 0; margin-left: 5px;">
				...
				foreach ( $plugins as $plugin ) {
					?>
							...
							if ( $plugin['doc'] ) {
								?>
										 <a style="font-size:12px" href="<?php echo $plugin['doc']; ?>" target="_blank"><?php _e( 'Documentation', 'acf_tooltip' ); ?></a>
									<?php
							}
							?>
							</li>
