...
class Api extends Singleton {
	...
	const
		...
		ROUTE_CONTAINER_RECURRING_ACTION_DELETE    = 'containers/recurring-action/delete',
		ROUTE_CONTAINER_RECURRING_CONTAINER_DELETE = 'containers/recurring-action/container/delete',

		ROUTE_DOMAIN_ADD                   = 'domain/add',
		ROUTE_DOMAIN_UPDATE                = 'domain/update',
		ROUTE_DOMAIN_CHECK_DEPLOYMENT_ZONE = 'domain/check/deployment-zone',
		ROUTE_DOMAIN_CHECK_ZONE            = 'domain/check/zone',
		ROUTE_DOMAIN_REMOVE                = 'domain/remove',
		ROUTE_DOMAIN_RECORDS_GET           = 'domain/records/get',
		ROUTE_DOMAIN_RECORDS_ADD           = 'domain/records/add',
		ROUTE_DOMAIN_RECORDS_REMOVE        = 'domain/records/remove',
		ROUTE_DOMAIN_RECORDS_EXISTING      = 'domain/records/existing',
	...
	public function process_token() {
		if ( isset( $_GET['data'], $_GET['page'] ) && $_GET['data'] && Options::PANEL_SLUG === $_GET['page'] ) {
			...
			if ( is_array( $data ) && isset( $data['token'], $data['domain'] ) && $data['token'] && $data['domain'] ) {
				...
				wp_redirect( admin_url( 'admin.php?page=' . Options::PANEL_SLUG ) );
				die();
			}
		}
	}
