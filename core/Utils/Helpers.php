...
class Helpers extends Singleton {
	...
	public function get_site_screenshot( $container_id = null ) {
		...
		if ( false === ( get_transient( 'dollie_site_new_screenshot_' . $post_id ) ) ) {
			$screenshot = $this->container_screenshot( $site );

			if ( ! empty( $screenshot ) && isset( $screenshot['desktop'] ) && $screenshot['desktop'] ) {
				set_transient( 'dollie_site_new_screenshot_' . $post_id, $screenshot, HOUR_IN_SECONDS );
			}
		} else {
			$screenshot = get_transient( 'dollie_site_new_screenshot_' . $post_id );
		}
