...
<div class="dol-border <?php do_action( 'dol_add_widget_classes' ); ?> dol-overflow-hidden dol-my-6">
	...
	<div class="dol-p-4 lg:dol-px-8 lg:dol-py-6 ">
		...
		<?php
		echo '<a class="dol-btn dol-btn-secondary dol-btn-sm dol-my-3 hover:dol-text-brand-600" href="sftp://' . $customer_details->containerSshUsername . ':' . $customer_details->containerSshPassword . '@' . $hostname . ':' . $customer_details->containerSshPort . '">' .
			dollie()->icon()->arrow_right( 'dol-mr-1' ) . esc_html__( 'Quick Connect!', 'dollie' ) . '</a>';
		?>
	</div>
</div>
