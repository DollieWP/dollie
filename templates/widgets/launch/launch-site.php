...
if ( dollie()->access()->size_limit_reached() ) {
	dollie()->load_template(
		'notice',
		array(
			'type'         => 'error',
...
if ( dollie()->access()->site_limit_reached() ) {
	dollie()->load_template(
		'notice',
		array(
			'type'         => 'error',
...
if ( dollie()->get_user()->can_manage_all_sites() && ! dollie()->get_partner()->has_partner_verified() ) {
	dollie()->load_template(
		'notice',
		array(
			'type'         => 'info',
...
if ( isset( $_GET['payment-status'] )
	...
	dollie()->load_template( 'widgets/launch/blueprint-launch', array( 'settings' => $settings ), true );

	return;
}

// Load form.
dollie()->load_template( 'widgets/launch/form', array( 'settings' => $settings ), true );
