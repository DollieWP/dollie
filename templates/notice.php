...
<div class="dol-border <?php do_action( 'dol_add_widget_classes' ); ?> dol-overflow-hidden <?php echo esc_attr($box_class); ?>">
	<div class="dol-flex dol-items-center <?php echo esc_attr($bar_class); ?>">
		...
		<?php endif; ?>
		<span class="dol-block lg:dol-py-2 dol-m-0 dol-p-0 dol-font-bold dol-text-white dol-text-sm md:dol-text-xl">
		...
		<?php if (!empty($args_default['links'])) : ?>
			<div>
				<?php foreach ($args_default['links'] as $link) : ?>
					...
					<a href="<?php echo esc_url($link['url']); ?>" target="<?php echo esc_attr($link_target); ?>" class="dol-text-sm dol-text-white hover:dol-text-white dol-inline-block dol-px-4 dol-py-2 dol-bg-secondary-500 hover:dol-bg-secondary-700 dol-rounded">
